...
`);
  ...
  invaders = Array(INVADER_ALIENS * INVADER_WAVES).fill(1);
...
function defeat() {
  ...
  if (defeat_after>0 && game.frameCount-defeat_after>3*game.frameRate) {
    game.fillStyle = "white";
    game.centerText(game.width/2, game.height/4*3, `SPACE to try again`);
...
function playgame() {
  ...
  // Game timer
  game.fillStyle = "white";
  game.drawText(game.width/2-6, 0, `${(game_timer_animation_remaining()/60)|0}`);
}
